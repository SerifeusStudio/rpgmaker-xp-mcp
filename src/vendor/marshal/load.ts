...
//
// FIXED (read_fixnum): upstream mis-decoded negative multibyte Fixnums.
// Ruby Marshal encodes e.g. -150 as [0xff, 0x6a]; the value is the unsigned
// little-endian bytes minus 2^(8*n). Upstream sign-extended from the top bit
// of the read bytes instead ((a << s) >> s), so -150 loaded as +106.
// This matters for RPG Maker data: every healing skill power < -123 hit it.
import * as constants from "./constants.js";
import { decode, defProp, define_extends, define_hash_default } from "./internal.js";
import {
  type Hash,
  RubyClass,
  RubyFloat,
...
const read_fixnum = (p: Loader): number => {
  ...
  if (t === 0) {
    ...
    // FIX: read |t| bytes as unsigned little-endian; negative form is the
    // unsigned value minus 2^(8*n) (matches Ruby marshal.c r_long).
    var n = Math.abs(t);
    var b = read_bytes(p, n);
    var a = 0;
    for (var i = n - 1; i >= 0; --i) a = a * 256 + b[i];
    return t > 0 ? a : a - 2 ** (8 * n);
  } else {
    return t > 0 ? t - 5 : t + 5;
  }
};
